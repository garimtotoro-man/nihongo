...
| 화면 | 내용 |
|---|---|
| 인트로(앱을 열 때마다) | 1단계 "왜 배우나요?" 팩 칩 복수 선택 → 2단계 레벨 [처음이에요(N5) | 조금 해봤어요(N4)] → 바로 피드. 이전 선택이 미리 체크돼 있다. 재방문자는 [홈으로] 로 건너뛸 수 있다. 같은 탭 안에서는 한 번만(sessionStorage) |
...
느낌은 레트로 도트(픽셀) 게임 UI 다(2026-09-18 결정). 글꼴은 Galmuri11(OFL, 한글·가나·영문 픽셀 폰트, `src/app/fonts/`), 모서리는 각지게(반지름 0), 테두리 2px 검정, 그림자는 오른쪽 아래 단색 3px, 버튼은 누르면 2px 눌린다. 배경은 옅은 청회색(`#e9eef0`) 위 24px 타일 격자, 진도 바는 칸이 나뉜 블록 게이지. 마스코트 맹구의 픽셀 그림과 같은 결이다.
...
- 명세 8.1의 "다음 목표 문자"는 글자를 그대로 보이면 정답이 노출되므로 한글 발음(`KanaChar.ko`, 예: 우 (u))으로 보인다. 짝·혼합 모드는 히라가나/가타카나, 탁음 모드는 청음/탁음 표시를 덧붙인다.
